...
import { getProperties, getWaterBills, saveWaterBills, getTenants, getCurrentMonth, formatCurrency } from '@/lib/api';
...
export default function WaterBillsPage() {
    ...
    useEffect(() => {
        ...
        const isIndividual = activeProp?.waterMeterSettings?.meterType === 'individual';
        const costPerUnit = parseFloat(activeProp?.waterMeterSettings?.costPerUnit || 0);
        ...
        const buildBills = (fetchedBills, prevReadingMap = {}) => {
            return fetchedBills.map(b => {
                ...
                const totalBill = currentReading === '' ? 0 : Math.round(isIndividual ? consumed * costPerUnit : currentReading);
        ...
        if (isFuture) {
            Promise.all([
                getWaterBills(selectedProp, month),
                getWaterBills(selectedProp, prevMonth)
            ]).then(([curRes, prevRes]) => {
                if (!active) return;
            ...
            getWaterBills(selectedProp, month).then(billRes => {
                if (!active) return;
    ...
    const handleReadingChange = (unitId, value) => {
        ...
        setBills(prev => prev.map(b => {
            ...
            const totalBill = value === '' ? 0 : Math.round(isIndividual ? consumed * costPerUnit : currentReading);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await saveWaterBills(selectedProp, { month, bills: formattedBills });
            setSaved(true);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 text-left">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-semibold tracking-[-0.02em] text-[#0F172A]">Water Bills</h2>
                    <p className="text-xs text-[#64748B] mt-1 uppercase tracking-widest">
                        Manage monthly water billing and utility consumptions for {displayMonth}
                        {isFuture && (
                                            ...
                                            <tr key={i} className="hover:bg-[#F8FAFC] transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    {isFuture ? (
                                                        ...
                                                        <span className="text-[11px] font-bold text-[#64748B] uppercase tracking-wider tabular-nums">
                                                            {previousReading.toLocaleString()} {isIndividual ? 'Units' : 'KES'}
                                                        </span>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <div className="space-y-0.5">
                                                        <span className="text-[13px] font-bold text-[#007AFF]">{formatCurrency(calculatedBill)}</span>
                                                        {isIndividual && consumed > 0 && (
                                                            <p className="text-[9px] font-bold text-[#94A3B8] uppercase tracking-wider">{consumed.toLocaleString()} Units used</p>
                                                        )}
                                                    </div>
                                                </td>
                                            </tr>
